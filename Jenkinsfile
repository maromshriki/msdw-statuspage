    ...
    stage('Dev Build') {
      ...
      steps {
        sshagent(credentials: ["$SSH_CREDENTIALS_ID_DEV"]) {
          ...
          sh "ssh -t $DEV_USER@$DEV_SERVER 'cd /opt/msdw-statuspage; docker build -t msdw/statuspage-web .; aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 992382545251.dkr.ecr.us-east-1.amazonaws.com; docker tag statuspage-web $REMOTE_REGISTRY:dev-test; docker push $REMOTE_REGISTRY:dev-test'"
        }
      }
    }
